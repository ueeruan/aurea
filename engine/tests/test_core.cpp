...
AUREA_TEST(Scheduler, FrameCacheEvictsLeastRecentlyUsed) {
    FrameCache cache;
    cache.set_capacity(3);
    for (u64 i = 0; i < 3; ++i) {
        FrameCache::Key k;
        k.hash = i + 1;
        cache.insert(k, TextureHandle{i + 1}, 10, i + 1);
    }
    // Toca o primeiro para ele deixar de ser o menos recente.
    FrameCache::Key k0;
    k0.hash = 1;
    cache.touch(k0, 100);
    ...
    FrameCache::Key k3;
    k3.hash = 99;
    cache.insert(k3, TextureHandle{99}, 10, 200);

    AUREA_CHECK_EQ(cache.count(), static_cast<u32>(3));
    AUREA_CHECK(cache.find(k0) != nullptr);   // sobreviveu
}

AUREA_TEST(Scheduler, PrefetchWindowFollowsDirection) {
    FramePrefetcher pf;
    std::vector<PrefetchRequest> requests;

    // Parado: janela simétrica.
    pf.set_direction(0);
    pf.plan(0, FrameIndex{100}, FrameIndex{0}, FrameIndex{1000}, requests);
    u32 ahead = 0, behind = 0;
    for (const auto& r : requests) {
        if (r.frame.value > 100) ++ahead;
        if (r.frame.value < 100) ++behind;
    }
    AUREA_CHECK(ahead > 0);
    AUREA_CHECK(behind > 0);

    // Arrastando para frente: a janela olha muito mais para a frente. Prefetch
    // para trás durante um arrasto para frente é trabalho jogado fora.
    pf.set_direction(1);
    pf.plan(0, FrameIndex{100}, FrameIndex{0}, FrameIndex{1000}, requests);
    ahead = 0; behind = 0;
    for (const auto& r : requests) {
    ...
    AUREA_CHECK(ahead > behind);
}

AUREA_TEST(Scheduler, PrefetchRespectsBounds) {
    FramePrefetcher pf;
    std::vector<PrefetchRequest> requests;
    pf.plan(0, FrameIndex{0}, FrameIndex{0}, FrameIndex{5}, requests);
    for (const auto& r : requests) {
        AUREA_CHECK(r.frame.value >= 0);
        AUREA_CHECK(r.frame.value <= 5);
    }
}
