...
const effectSpecs = <EffectType, EffectSpec>{
  ...
  // ---------------------------------------------------------------
  // GERADORES (v1.1.1). Desenho que nasce na propria camada: nuvem,
  // xadrez, listra, ponto, estrela e raio. Todos tem MISTURA (quanto do
  // desenho entra) e RECORTAR (em vez de pintar, o desenho vira o
  // recorte da camada) — e assim o mesmo gerador serve de fundo, de
  // textura por cima e de mascara.
  // ---------------------------------------------------------------
  EffectType.nuvens: EffectSpec(
    id: 'clouds',
    name: 'Clouds',
    category: 'Generate',
    hasColor: true,
    defaultColor: Color(0xFF0E1A2B),
    extraColors: 1,
    defaultExtraColors: [Color(0xFFFFFFFF)],
    synonyms: ['nuvem', 'nuvens', 'fumaca', 'neblina', 'fractal', 'clouds'],
    cost: 3,
    params: {
      'escala': EffectParam('Tamanho', 3.0, .2, 20.0),
      'detalhe': EffectParam('Detalhe', 5, 1, 8),
      'evolucao': EffectParam('Evolucao', 1.0, 0.0, 10.0),
      'contraste': EffectParam('Contraste', .5, 0, 1),
      'semente': EffectParam('Semente', 1, 1, 99),
      'mistura': EffectParam('Mistura', 1, 0, 1),
      'recortar': EffectParam('Recortar', 0, 0, 1, kind: ParamKind.toggle),
    },
    montar: ['escala', 'evolucao'],
    presets: [
      EffectPronto('Neblina', {'escala': 6, 'contraste': .25}),
      EffectPronto('Fumaca', {'escala': 2, 'evolucao': 3, 'contraste': .8}),
      EffectPronto('Marmore', {'escala': 9, 'detalhe': 8, 'contraste': .9}),
    ],
  ),
  EffectType.xadrez: EffectSpec(
    id: 'checker',
    name: 'Checker',
    category: 'Generate',
    hasColor: true,
    defaultColor: Color(0xFF12151A),
    extraColors: 1,
    defaultExtraColors: [Color(0xFFFFFFFF)],
    synonyms: ['xadrez', 'tabuleiro', 'quadros', 'checker', 'padrao'],
    params: {
      'quantidade': EffectParam('Quadros', 8.0, .5, 60.0),
      'esticar': EffectParam('Esticar', 1.0, .1, 6.0),
      'angulo': EffectParam('Angulo', 0, -180, 180),
      'mistura': EffectParam('Mistura', 1, 0, 1),
      'recortar': EffectParam('Recortar', 0, 0, 1, kind: ParamKind.toggle),
    },
    montar: ['quantidade', 'angulo'],
    presets: [
      EffectPronto('Xadrez fino', {'quantidade': 24}),
      EffectPronto('Diagonal', {'angulo': 45, 'quantidade': 12}),
      EffectPronto('Tijolo', {'esticar': 2.5, 'quantidade': 10}),
    ],
  ),
  EffectType.listras: EffectSpec(
    id: 'stripes',
    name: 'Stripes',
    category: 'Generate',
    hasColor: true,
    ...
    extraColors: 1,
    defaultExtraColors: [Color(0xFFFFFFFF)],
    synonyms: ['listra', 'listras', 'faixas', 'barras', 'stripes', 'risca'],
    params: {
      'quantidade': EffectParam('Listras', 10.0, .5, 80.0),
      'angulo': EffectParam('Angulo', 0, -180, 180),
      'proporcao': EffectParam('Proporcao', .5, .05, .95),
      'suavidade': EffectParam('Suavidade', .05, 0, 1),
      'deslocamento': EffectParam('Deslocamento', 0, -1, 1),
      'mistura': EffectParam('Mistura', 1, 0, 1),
      'recortar': EffectParam('Recortar', 0, 0, 1, kind: ParamKind.toggle),
    },
    montar: ['quantidade', 'angulo'],
    presets: [
      EffectPronto('Persiana', {'quantidade': 24, 'proporcao': .3}),
      EffectPronto('Degrade', {'suavidade': 1, 'quantidade': 6}),
      EffectPronto('Zebra', {'quantidade': 14, 'angulo': 70}),
    ],
  ),
  EffectType.pontos: EffectSpec(
    id: 'dots',
    name: 'Dots',
    category: 'Generate',
    hasColor: true,
    ...
    extraColors: 1,
    defaultExtraColors: [Color(0xFFFFFFFF)],
    synonyms: ['ponto', 'pontos', 'bolinhas', 'meio-tom', 'dots', 'trama'],
    params: {
      'quantidade': EffectParam('Pontos', 16.0, 1.0, 80.0),
      'tamanho': EffectParam('Tamanho', .5, .05, 1.4),
      'suavidade': EffectParam('Suavidade', .15, 0, 1),
      'angulo': EffectParam('Angulo', 0, -180, 180),
      'mistura': EffectParam('Mistura', 1, 0, 1),
      'recortar': EffectParam('Recortar', 0, 0, 1, kind: ParamKind.toggle),
    },
    montar: ['quantidade', 'tamanho'],
    presets: [
      EffectPronto('Trama', {'quantidade': 40, 'tamanho': .4, 'angulo': 45}),
      EffectPronto('Bolotas', {'quantidade': 8, 'tamanho': .9}),
      EffectPronto('Poeira', {'quantidade': 70, 'tamanho': .12}),
    ],
  ),
  EffectType.estrelas: EffectSpec(
    id: 'starfield',
    name: 'Starfield',
    category: 'Generate',
    hasColor: true,
    defaultColor: Color(0xFF05070C),
    extraColors: 1,
    defaultExtraColors: [Color(0xFFFFFFFF)],
    synonyms: ['estrela', 'estrelas', 'ceu', 'noite', 'brilho', 'starfield'],
    cost: 2,
    params: {
      'quantidade': EffectParam('Estrelas', 40.0, 4.0, 200.0),
      'tamanho': EffectParam('Tamanho', .35, .05, 1.0),
      'cintilar': EffectParam('Cintilar', .5, 0, 1),
      'velocidade': EffectParam('Velocidade', .2, 0, 4),
      'semente': EffectParam('Semente', 1, 1, 99),
      'mistura': EffectParam('Mistura', 1, 0, 1),
      'recortar': EffectParam('Recortar', 0, 0, 1, kind: ParamKind.toggle),
    },
    montar: ['quantidade', 'cintilar'],
    presets: [
      EffectPronto('Ceu limpo', {'quantidade': 120, 'tamanho': .18}),
      EffectPronto('Neve', {'quantidade': 60, 'tamanho': .7, 'velocidade': 1}),
      EffectPronto('Vaga-lume', {'quantidade': 20, 'cintilar': 1}),
    ],
  ),
  EffectType.raios: EffectSpec(
    id: 'radial_rays',
    name: 'Radial Rays',
    category: 'Generate',
    hasColor: true,
    defaultColor: Color(0x0012151A),
    extraColors: 1,
    defaultExtraColors: [Color(0xFFFFE9A8)],
    synonyms: ['raio', 'raios', 'sol', 'explosao', 'leque', 'rays'],
    cost: 2,
    params: {
      'quantidade': EffectParam('Raios', 12.0, 2.0, 80.0),
      'centro_x': EffectParam('Centro X', .5, -.5, 1.5),
      'centro_y': EffectParam('Centro Y', .5, -.5, 1.5),
      'largura': EffectParam('Largura', .5, .05, .95),
      'suavidade': EffectParam('Suavidade', .3, 0, 1),
      'giro': EffectParam('Giro', 0, -180, 180),
      'alcance': EffectParam('Alcance', 1.0, .1, 3.0),
      'mistura': EffectParam('Mistura', 1, 0, 1),
      'recortar': EffectParam('Recortar', 0, 0, 1, kind: ParamKind.toggle),
    },
    montar: ['quantidade', 'giro'],
    presets: [
      EffectPronto('Sol', {'quantidade': 24, 'largura': .3, 'alcance': 1.6}),
      EffectPronto('Explosao', {'quantidade': 48, 'suavidade': .1}),
      EffectPronto('Holofote', {'quantidade': 4, 'largura': .12, 'alcance': 2}),
    ],
  ),

  // ---------------------------------------------------------------
  // CORTINA, RECORTE E BORDA (v1.1.1). O que faltava para terminar um
  // recorte: revelar a camada por uma linha, comer a franja que sobra
  // do chroma, por contorno, brilho por dentro e beirada rasgada.
  // ---------------------------------------------------------------
  EffectType.cortina: EffectSpec(
    id: 'wipe',
    name: 'Wipe',
    category: 'Keying',
    synonyms: ['cortina', 'revelar', 'varrer', 'wipe', 'entrada', 'limpar'],
    params: {
      'progresso': EffectParam('Avanco', .5, 0, 1),
      'angulo': EffectParam('Angulo', 0, -180, 180),
      'suavidade': EffectParam('Suavidade', .05, 0, 1),
      'inverter': EffectParam('Inverter', 0, 0, 1, kind: ParamKind.toggle),
    },
    montar: ['progresso', 'angulo'],
    presets: [
      EffectPronto('De cima', {'angulo': 90, 'progresso': .5}),
      EffectPronto('Diagonal macia', {'angulo': 45, 'suavidade': .4}),
      EffectPronto('Revelar', {'progresso': .8, 'suavidade': .15}),
    ],
  ),
  EffectType.cortinaRadial: EffectSpec(
    id: 'radial_wipe',
    name: 'Radial Wipe',
    category: 'Keying',
    synonyms: [
    ...
    params: {
      'progresso': EffectParam('Avanco', .5, 0, 1),
      'comeco': EffectParam('Comeco', 90, -180, 180),
      'suavidade': EffectParam('Suavidade', .02, 0, 1),
      'sentido': EffectParam(
        'Sentido',
        0,
        0,
        1,
        kind: ParamKind.choice,
        options: ['Horario', 'Anti-horario'],
      ),
      'centro_x': EffectParam('Centro X', .5, -.5, 1.5),
      'centro_y': EffectParam('Centro Y', .5, -.5, 1.5),
    },
    montar: ['progresso', 'comeco'],
    presets: [
      EffectPronto('Relogio', {'progresso': .5}),
      EffectPronto('Contagem', {'progresso': .25, 'comeco': 90}),
      EffectPronto('Anti-horario', {'sentido': 1, 'progresso': .6}),
    ],
  ),
  EffectType.apertarRecorte: EffectSpec(
    id: 'matte_choker',
    name: 'Matte Choker',
    category: 'Keying',
    synonyms: [
    ...
    cost: 2,
    params: {
      'aperto': EffectParam('Aperto', 6.0, -40.0, 40.0, relative: true),
      'suavidade': EffectParam('Suavidade', .4, 0, 1),
    },
    montar: ['aperto'],
    presets: [
      EffectPronto('Comer a franja', {'aperto': 8}),
      EffectPronto('Devolver a borda', {'aperto': -8}),
      EffectPronto('Aperto macio', {'aperto': 4, 'suavidade': 1}),
    ],
  ),
  EffectType.meioTom: EffectSpec(
    id: 'halftone',
    name: 'Halftone',
    category: 'Stylize',
    hasColor: true,
    ...
    cost: 2,
    params: {
      'quantidade': EffectParam('Pontos', 40.0, 4.0, 160.0),
      'angulo': EffectParam('Angulo', 45, -180, 180),
      'suavidade': EffectParam('Suavidade', .2, 0, 1),
      'mistura': EffectParam('Mistura', 1, 0, 1),
    },
    montar: ['quantidade', 'angulo'],
    presets: [
      EffectPronto('Jornal', {'quantidade': 70}),
      EffectPronto('Quadrinho', {'quantidade': 26, 'suavidade': .05}),
      EffectPronto('Meia forca', {'mistura': .5, 'quantidade': 50}),
    ],
  ),
  EffectType.contorno: EffectSpec(
    id: 'outline',
    name: 'Outline',
    category: 'Stylize',
    hasColor: true,
    ...
    cost: 2,
    params: {
      'espessura': EffectParam('Espessura', 12.0, 0.0, 80.0, relative: true),
      'suavidade': EffectParam('Suavidade', .4, 0, 1),
      'so_contorno': EffectParam(
        'So o contorno',
        0,
        0,
        1,
    ...
    montar: ['espessura'],
    presets: [
      EffectPronto('Adesivo', {'espessura': 20}),
      EffectPronto('Fio', {'espessura': 4, 'suavidade': .1}),
      EffectPronto('So a linha', {'so_contorno': 1, 'espessura': 10}),
    ],
  ),
  EffectType.brilhoPorDentro: EffectSpec(
    id: 'inner_glow',
    name: 'Inner Glow',
    category: 'Light',
    hasColor: true,
    defaultColor: Color(0xFFFFE9A8),
    synonyms: [
      'brilho interno',
      'brilho por dentro',
      'inner glow',
    ...
    cost: 2,
    params: {
      'tamanho': EffectParam('Tamanho', 24.0, 0.0, 120.0, relative: true),
      'forca': EffectParam('Forca', 1.0, 0, 4),
    },
    montar: ['tamanho', 'forca'],
    presets: [
      EffectPronto('Neon', {'tamanho': 16, 'forca': 2}),
      EffectPronto('Vidro', {'tamanho': 60, 'forca': .8}),
      EffectPronto('Beirada', {'tamanho': 8, 'forca': 3}),
    ],
  ),
  EffectType.bordasAsperas: EffectSpec(
    id: 'roughen_edges',
    name: 'Roughen Edges',
    category: 'Distort',
    synonyms: [
    ...
    cost: 2,
    params: {
      'tamanho': EffectParam('Tamanho', 14.0, 0.0, 80.0, relative: true),
      'escala': EffectParam('Miudeza', 8.0, .5, 40.0),
      'evolucao': EffectParam('Evolucao', 0.0, 0.0, 10.0),
      'semente': EffectParam('Semente', 1, 1, 99),
    },
    montar: ['tamanho', 'escala'],
    presets: [
      EffectPronto('Rasgado', {'tamanho': 24, 'escala': 5}),
      EffectPronto('Carimbo', {'tamanho': 10, 'escala': 20}),
      EffectPronto('Fogo', {'tamanho': 18, 'escala': 12, 'evolucao': 3}),
    ],
  ),
};
