...
({AnimatedDouble x, AnimatedDouble y}) _escala(
  double base, [
...
ShapeLayer _circulo({
  ...
  opacity: opacidade,
  contents: [
    ShapeParametric(kind: ParamShapeKind.ellipse, sizeX: _ad(d), sizeY: _ad(d)),
...
VideoProject buildNotesMotionTemplate() {
  ...
  // clica (o pill afunda e volta); tudo sai encolhendo e borrado.
  {
    ...
    camadas.add(
      ShapeLayer(
        ...
        contents: [
          ShapeSvgPath(
            pathData: 'M0 0 L0 17 L4.2 13 L7.2 19.5 L9.6 18.4 L6.7 12 L12 12 Z',
            size: 66,
          ),
          ShapeFill(color: _branco),
  ...
  // fim cresce mais e some (zoom para o menu), com whip para a esquerda.
  {
    ...
    final filhos = <Layer>[
      ...
      ...linha('c_l3', 'Countries to Visit', 'Wednesday', 'Trips', 700).layers,
      ...linha(
        'c_l4',
        'TV Shows to Rewatch',
        'Tuesday   Santa Clarita Diet',
        'Ming House',
        790,
      ).layers,
      ...linha(
      ...
      for (final (i, it) in itens.indexed) ...[
        _texto(
          ...
          posicao: _ao(Offset(330 + it.length * 6.3, 452 + i * 44.0), [
            (
              6.45 + i * 0.04,
              Offset(430 + it.length * 6.3, 452 + i * 44.0),
              _entra,
            ),
            (
              6.85 + i * 0.04,
              Offset(330 + it.length * 6.3, 452 + i * 44.0),
              _entra,
            ),
          ]),
  ...
  // entram com desfoque; tudo sai crescendo e borrado.
  {
    ...
    for (final (i, linha) in [
      'You finished your grocery list',
      'for the day!',
    ].indexed) {
      camadas.add(
        _texto(
