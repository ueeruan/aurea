// AUREA: copia de rife-ncnn-vulkan a7532fc (MIT, nihui) com as correcoes
// descritas em PATCHES.md. Nao editar sem atualizar la.
...
// AUREA: devolve o erro (0 = ok). Caminho longo demais tambem e erro.
#if _WIN32
static int load_param_model(ncnn::Net& net, const std::wstring& modeldir, const wchar_t* name)
{
    wchar_t parampath[1024];
    wchar_t modelpath[1024];
    if (swprintf(parampath, 1024, L"%s/%s.param", modeldir.c_str(), name) < 0) return -1;
    if (swprintf(modelpath, 1024, L"%s/%s.bin", modeldir.c_str(), name) < 0) return -1;

    {
        FILE* fp = _wfopen(parampath, L"rb");
        if (!fp) return -1;
        const int r = net.load_param(fp);
        fclose(fp);
        if (r != 0) return -1;
    }
    {
...
static int load_param_model(ncnn::Net& net, const std::string& modeldir, const char* name)
{
    char parampath[1024];
    char modelpath[1024];
    const int a = snprintf(parampath, sizeof(parampath), "%s/%s.param", modeldir.c_str(), name);
    const int b = snprintf(modelpath, sizeof(modelpath), "%s/%s.bin", modeldir.c_str(), name);
    if (a < 0 || a >= (int)sizeof(parampath) || b < 0 || b >= (int)sizeof(modelpath)) return -1;

    if (net.load_param(parampath) != 0) return -1;
    if (net.load_model(modelpath) != 0) return -1;
    return 0;
}
#endif
...
#endif
{
    ...
    // AUREA: so com GPU (o ncnn novo le vkdev->info aqui; nulo derrubava a CPU)
    if (vkdev)
    {
...
#if _WIN32
    if (load_param_model(flownet, modeldir, L"flownet") != 0) return -1;
    if (!rife_v4)
    {
        if (load_param_model(contextnet, modeldir, L"contextnet") != 0) return -1;
        if (load_param_model(fusionnet, modeldir, L"fusionnet") != 0) return -1;
    }
#else
    if (load_param_model(flownet, modeldir, "flownet") != 0) return -1;
    if (!rife_v4)
    {
        if (load_param_model(contextnet, modeldir, "contextnet") != 0) return -1;
        if (load_param_model(fusionnet, modeldir, "fusionnet") != 0) return -1;
    }
#endif

    // AUREA (PATCH 3): Net::load_param desliga fp16/int8 que a GPU nao tem
    // em flownet.opt, e process_v4 le flownet.opt. Os shaders abaixo eram
    // compilados com a opcao ANTES dessa limpeza: numa GPU sem int8 storage
    // o preproc esperava uint8 e recebia float.
    opt = flownet.opt;
    if (vkdev && !opt.use_vulkan_compute) return -1;
    ...
    if (vkdev)
    {
...

        {
            ...
            if (spirv.empty() || rife_preproc->create(spirv.data(), spirv.size() * 4, specializations) != 0) return -1;
        }

        {
            ...
            if (spirv.empty() || rife_postproc->create(spirv.data(), spirv.size() * 4, specializations) != 0) return -1;
        }
    }
    ...
    if (rife_v4)
    {
        if (vkdev)
        {
            ...
            if (spirv.empty() || rife_v4_timestep->create(spirv.data(), spirv.size() * 4, specializations) != 0) return -1;
        }
    }
...
//     fprintf(stderr, "%d x %d\n", w, h);
    ...
    else
    {
        ...
        else
        {
            ...
            ex.input("in1", in1_gpu_padded);
            ex.input("in2", timestep_gpu_padded);
            if (ex.extract("out0", out_gpu_padded, cmd) != 0 || out_gpu_padded.empty())
            {
                vkdev->reclaim_blob_allocator(blob_vkallocator);
                vkdev->reclaim_staging_allocator(staging_vkallocator);
                return -1;
    ...
    // download
    {
        ...
        if (out_gpu.empty() || cmd.submit_and_wait() != 0 || out.empty())
        {
            vkdev->reclaim_blob_allocator(blob_vkallocator);
            vkdev->reclaim_staging_allocator(staging_vkallocator);
            return -1;
...
//     fprintf(stderr, "%d x %d\n", w, h);
    ...
    else
    {
        ...
        else
        {
            ...
            ex.input("in1", in1_padded);
            ex.input("in2", timestep_padded);
            if (ex.extract("out0", out_padded) != 0 || out_padded.empty()) return -1;
        }
