...
""");
    ...
    final srcs = File.fromUri(outputDirectory.resolve('thermion_sources.rsp'));
    srcs.parent.createSync(recursive: true);
    srcs.writeAsStringSync(sources.map((source) => '"$source"').join('\n'));
    ...
    // Windows passes sources through a response file, so CBuilder cannot infer
    // these dependencies. Track them to avoid running stale native code.
    output.dependencies.addAll(sources.map((source) => File(source).uri));
    output.dependencies.addAll(Directory(path.join(pkgRootFilePath, 'native', 'include', 'c_api'))
      .listSync().whereType<File>().map((file) => file.uri));
    ...
    if (targetOS == OS.windows) {
      final importLib = File.fromUri(outputDirectory.resolve('thermion_dart.lib'));
