...
extern "C"
{
  ...
  // Aurea: let Filament's GPU timing controller scale its internal render
  // target without reallocating the Flutter presentation surface every frame.
  // One task keeps begin/render/end atomic across viewers sharing an engine.
  // Respect GPU backpressure and never insert a CPU/GPU fence in preview.
  EMSCRIPTEN_KEEPALIVE void Aurea_renderPreviewFrameRenderThread(
      TRenderer *tRenderer, TView *tView, TSwapChain *tSwapChain, void (*onComplete)(bool))
  {
    auto *rt = RT(tRenderer);
    std::packaged_task<void()> task([=] {
      auto *renderer = reinterpret_cast<filament::Renderer *>(tRenderer);
      const bool accepted = renderer->beginFrame(reinterpret_cast<filament::SwapChain *>(tSwapChain));
      if (accepted) {
        renderer->render(reinterpret_cast<filament::View *>(tView));
        renderer->endFrame();
      }
      PROXY(onComplete(accepted));
    });
    rt->addTask(task);
  }

  EMSCRIPTEN_KEEPALIVE void Aurea_configureDynamicResolutionRenderThread(
      TView *tView, TRenderer *tRenderer, bool enabled, float minimumScale,
      uint32_t requestId, VoidCallback onComplete)
  {
    auto *rt = RT(tView);
    std::packaged_task<void()> task([=] {
      ...
      auto *renderer = reinterpret_cast<filament::Renderer *>(tRenderer);
      filament::DynamicResolutionOptions options;
      options.enabled = enabled;
      options.homogeneousScaling = true;
      options.minScale = {minimumScale, minimumScale};
      options.maxScale = {1.0f, 1.0f};
      view->setDynamicResolutionOptions(options);
      filament::Renderer::FrameRateOptions pacing;
      pacing.headRoomRatio = 0.15f;
      pacing.history = 30;
      pacing.scaleRate = 1.0f / 16.0f;
      renderer->setFrameRateOptions(pacing);
      PROXY(onComplete(requestId));
    });
    rt->addTask(task);
  }

  EMSCRIPTEN_KEEPALIVE void Aurea_getResolutionScaleRenderThread(TView *tView, void (*onComplete)(float))
  {
    auto *rt = RT(tView);
    std::packaged_task<void()> task([=] {
      const float scale = reinterpret_cast<filament::View *>(tView)->getLastDynamicResolutionScale().x;
      PROXY(onComplete(scale));
    });
    rt->addTask(task);
  }

  EMSCRIPTEN_KEEPALIVE void Aurea_getGpuDurationRenderThread(TRenderer *tRenderer, void (*onComplete)(float))
  {
    auto *rt = RT(tRenderer);
    std::packaged_task<void()> task([=] {
      const auto history = reinterpret_cast<filament::Renderer *>(tRenderer)->getFrameInfoHistory(32);
      float milliseconds = -1.0f;
      uint32_t latest = 0;
      for (const auto &frame : history) {
        if (frame.gpuFrameDuration >= 0 && frame.frameId >= latest) {
          latest = frame.frameId;
          milliseconds = static_cast<float>(frame.gpuFrameDuration / 1000000.0);
        }
      }
      PROXY(onComplete(milliseconds));
    });
    rt->addTask(task);
  }
