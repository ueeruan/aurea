...
void main() {
  group('plano', () {
    test('30 -> 60: um quadro do meio entre cada par, exatamente t = 1/2', () {
      final p = planoDeInterpolacao(quadrosBase: 3, taxaBase: 30, taxaSaida: 60);
      expect(p, const [
    ...
    test('30 -> 120: quartos exatos e sem deriva no fim de um clipe longo', () {
      final p = planoDeInterpolacao(quadrosBase: 900, taxaBase: 30, taxaSaida: 120);
    ...
    test('24 -> 60: instantes fracionarios corretos (i*24/60)', () {
      final p = planoDeInterpolacao(quadrosBase: 24, taxaBase: 24, taxaSaida: 60);
      expect(p.length, 60);
    ...
    test('bordas: sem quadros ou taxa invalida devolve vazio; um quadro so copia', () {
      expect(planoDeInterpolacao(quadrosBase: 0, taxaBase: 30, taxaSaida: 60), isEmpty);
      expect(planoDeInterpolacao(quadrosBase: 5, taxaBase: 0, taxaSaida: 60), isEmpty);
      expect(planoDeInterpolacao(quadrosBase: 1, taxaBase: 30, taxaSaida: 120),
          everyElement(predicate<PassoDeInterpolacao>((p) => p.copia && p.a == 0)));
    });
  });

  group('estrategia', () {
    test('clipe sem camera lenta: extracao normal', () {
      final e = estrategiaDeInterpolacao(_clipe(), fps: 30, fpsDaFonte: 30, rifeDisponivel: true);
    ...
    test('fonte de 60 fps a 50% numa composicao de 30: quadros reais, sem inventar', () {
      final e = estrategiaDeInterpolacao(_clipe(speed: .5), fps: 30, fpsDaFonte: 59.94, rifeDisponivel: true);
      expect(e.como, ComoInterpolar.quadrosReais);
      expect(e.taxaBase, 60);
    });

    test('fonte de 30 a 25% com o motor: RIFE lendo a fonte a 30', () {
      final e = estrategiaDeInterpolacao(_clipe(speed: .25), fps: 30, fpsDaFonte: 29.97, rifeDisponivel: true);
      expect(e.como, ComoInterpolar.rife);
      expect(e.taxaBase, 30);
    });

    test('24 fps a 50% em composicao de 30: RIFE de 24 para 60', () {
      final e = estrategiaDeInterpolacao(_clipe(speed: .5), fps: 30, fpsDaFonte: 23.976, rifeDisponivel: true);
      expect(e.como, ComoInterpolar.rife);
    ...
    test('sem motor, ou no modo mesclar: FFmpeg como sempre', () {
      expect(
        estrategiaDeInterpolacao(_clipe(speed: .5), fps: 30, fpsDaFonte: 30, rifeDisponivel: false).como,
    ...
    test('taxa da fonte desconhecida vale a da composicao', () {
      final e = estrategiaDeInterpolacao(_clipe(speed: .5), fps: 30, fpsDaFonte: null, rifeDisponivel: true);
      expect(e.como, ComoInterpolar.rife);
      expect(e.taxaBase, 30);
      final absurda = estrategiaDeInterpolacao(_clipe(speed: .5), fps: 30, fpsDaFonte: 90000, rifeDisponivel: true);
    ...
    test('efeito Optical Flow desligado anula a interpolacao do clipe', () {
      ...
      expect(
        estrategiaDeInterpolacao(c, fps: 30, fpsDaFonte: 30, rifeDisponivel: true).como,
        ComoInterpolar.nada,
      );
    });
  });
  ...
  group('taxa da fonte pelo ffprobe', () {
    test('razoes, media antes da nominal e valores invalidos', () {
      expect(fpsDeProps({'avg_frame_rate': '30000/1001'}), closeTo(29.97, 1e-3));
      expect(fpsDeProps({'avg_frame_rate': '0/0', 'r_frame_rate': '60/1'}), 60);
      expect(fpsDeProps({'avg_frame_rate': '240/1', 'r_frame_rate': '30/1'}), 240);
