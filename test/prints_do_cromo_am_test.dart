// PRINTS DO CROMO AM (v1.1.1, sem emulador): o editor na planta do AM 5
// — navbar, playbar, trilho do palco, barra da selecao, selectbar verde
...
import 'package:aurea/src/features/editor/presentation/shell/cromo_am.dart';
...
void main() {
  ...
  testWidgets('o editor na planta do AM monta sem estouro', (tester) async {
    final (_, chave) = await _editor(tester);
    expect(tester.takeException(), isNull);
    // As pecas do AM no lugar: navbar com relogio, playbar, trilho.
    ...
    await gravarPrint(tester, chave, 'am-editor');
  });
  ...
  testWidgets('camada selecionada: a barra flutuante de aparar/dividir', (
    ...
    await gravarPrint(tester, chave, 'am-editor-selecao');
  });

  testWidgets('multi-selecao: a selectbar verde assume o topo', (
    tester,
  ) async {
    final (c, chave) = await _editor(tester);
    ...
    await tester.pumpAndSettle();
    expect(tester.takeException(), isNull);
    expect(find.byType(AmSelectbar), findsOneWidget);
    expect(find.byKey(const ValueKey('selectbar-agrupar')), findsOneWidget);
    expect(
      find.byKey(const ValueKey('selectbar-alinhar-esquerda')),
      findsOneWidget,
    );
    await gravarPrint(tester, chave, 'am-editor-multi');
  });

  testWidgets('o + abre o popup de adicionar com as abas do AM', (
    tester,
  ) async {
    final (_, chave) = await _editor(tester);
    ...
    await gravarPrint(tester, chave, 'am-editor-adicionar');
  });
}
